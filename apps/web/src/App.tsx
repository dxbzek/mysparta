...
export function App() {
  ...
  if (!save || screen.s === "forge") {
    return (
      <Shell>
        <Forge
          onForge={(champion, hero, styleFx, aura) => {
            const s = newSave(champion, { stance: "measured", gambit: "close_the_gap" }, hero, styleFx, aura);
            update(s);
            setScreen({ s: "home" });
          }}
        />
...
function Forge({
  ...
  onForge: (c: Champion, hero: number, styleFx: number, aura: number) => void;
}) {
